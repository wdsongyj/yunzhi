<%@ Page Title="" Language="C#" MasterPageFile="~/Manage/List.Master" AutoEventWireup="true" CodeBehind="YZHospitalList.aspx.cs" Inherits="YUNZHI.Management.Manage.YZHospitalList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolderMain" runat="server">
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 医疗机构管理 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px" href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="page-container">
        <div class="text-c">
            <span class="text-serch-f">医疗机构名称：</span><asp:TextBox ID="txtTitle" runat="server" placeholder=" " Style="width: 250px" class="input-text"></asp:TextBox>
            <span class="text-serch">机构代码：</span><asp:TextBox ID="txtCode" runat="server" placeholder=" " Style="width: 250px" class="input-text"></asp:TextBox>
            <span class="text-serch">区域：</span><%--<asp:DropDownList ID="ddlAreaType" runat="server" CssClass="select-box select radius" size="1" Style="width: 120px"></asp:DropDownList>--%>
        ...
        <div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l">
                ...
                <a class="btn btn-primary radius ml-5" onclick="article_add('添加','YZHospitalManage.aspx')" href="javascript:;" title="添加"><i class="Hui-iconfont">&#xe600;</i> 添加医疗机构</a></span>
            <span class="l ml-10">
                ...
                <tbody>
                    <asp:Repeater ID="RepLetter" runat="server">
                        <ItemTemplate>
                            <tr class="text-c">
                                <td>
                                    <input type="checkbox" value="" id="cbId" ubi="<%# Eval("HID") %>" onclick="GetSelectCheckBox(this);" name="ubiid"></td>
                                <td><%# Eval("HCode") %></td>
                                <td class="text-l">
                                    <u style="cursor: pointer" class="text-primary" onclick="article_view('查看','YZHospitalManage.aspx','<%# Eval("HID") %>')" title="<%# Eval("HName") %>">
                                        <%# Eval("HName") %></u>
                                </td>
                                <td><%# Eval("AreaProvince") %></td>
                                <td><%# Eval("AreaUrban") %></td>
                                <td class="f-14 td-manage">
                                    <div id="bntdiv" runat="server">
                                        <a style="text-decoration: none" class="ml-5" onclick="article_edit('编辑','YZHospitalManage.aspx','<%# Eval("HID") %>')" href="javascript:;" title="管理"><i class="Hui-iconfont">&#xe642;</i></a>
                                        <asp:LinkButton ID="lbtnDel" Style="text-decoration: none" ClientIDMode="AutoID" class="ml-5" OnClientClick='return article_del(this,"ID")' runat="server" title="删除" CommandName='<%# Eval("HID") %>' OnCommand="lbtnDel_Command"><i class="Hui-iconfont">&#xe6e2;</i></asp:LinkButton>
                                    </div>
                                </td>
                            </tr>
                        </ItemTemplate>
